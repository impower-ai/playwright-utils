...
import * as TimingUtils from '../../../lib/utils/timing/timing';

describe('Timing Utilities', () => {

    test('createTimeoutPromise', async () => {
        const promise = TimingUtils.createTimeoutPromise(100);
    ...
    test('createSafeCondition with successful condition', async () => {
        ...
        const result = await TimingUtils.createSafeCondition(condition);
    ...
    test('waitFor with successful condition', async () => {
        ...
        await expect(TimingUtils.waitFor(condition, { pollInterval: 10 })).resolves.not.toThrow();
        expect(counter).toBe(3);
    });

    test('waitFor with timeout', async () => {
        const condition = async () => false;
        await expect(TimingUtils.waitFor(condition, { timeout: 100, pollInterval: 10 }))
    ...
    test('waitForStableValue with quickly stabilizing value', async () => {
        ...
        const result = await TimingUtils.waitForStableValue(valueFunc, { pollInterval: 10 });
    ...
    test('waitForStableValue with timeout', async () => {
        ...
        await expect(TimingUtils.waitForStableValue(valueFunc, { timeout: 100, pollInterval: 10 }))
    ...
    test('waitForAll with all conditions succeeding', async () => {
        ...
        await expect(TimingUtils.waitForAll(conditions)).resolves.not.toThrow();
    });
    ...
    test('waitForAll with one condition failing', async () => {
        ...
        await expect(TimingUtils.waitForAll(conditions)).rejects.toThrow(ConditionError);
    });
    ...
    test('waitForAny with one condition succeeding', async () => {
        ...
        await expect(TimingUtils.waitForAny(conditions)).resolves.not.toThrow();
    });
    ...
    test('waitForAny with all conditions failing', async () => {
        ...
        await expect(TimingUtils.waitForAny(conditions)).rejects.toThrow(ConditionError);
    });

    test('waitForEither with first condition succeeding', async () => {
        const condition1 = async () => true;
        const condition2 = async () => false;
        await expect(TimingUtils.waitForEither(condition1, condition2)).resolves.not.toThrow();
    });
    ...
    test('waitForEither with both conditions failing', async () => {
        const condition1 = async () => false;
        const condition2 = async () => false;
        await expect(TimingUtils.waitForEither(condition1, condition2, { timeout: 100, pollInterval: 10 }))
            .rejects.toThrow(ConditionError);
    });
});