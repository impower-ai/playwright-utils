 ...
 */
export function isFlagEnum(enumObj: object): boolean {
 ...
 */
export function isValidEnumValue(value: any, enumType: object): boolean {
 ...
 */
export function getEnumValues(enumObj: object): string[] | number[] {
    if (!isFlagEnum(enumObj))
 ...
 * @template T
 */
export function getEnumString<T extends { [key: string]: string | number }>(enumObj: T, flags: number): string {
    if (flags === 0) 
 ...
 */
export function validateEnumProperty(json: any, property: string, enumType: object): number | string | null {
